...
export const Designations = () => {
    ...
    const [formData, setFormData] = useState({ title: '', department_id: '', designation_code: '', description: '', status: 'Active' });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setFormData({ title: '', department_id: '', designation_code: '', description: '', status: 'Active' });
            setEditingId(null);
            fetchData();
        } catch (err) {
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-8">
                ...
                {isAdminOrHR && (
                    <button 
                        onClick={() => { setFormData({title: '', department_id: '', designation_code: '', description: '', status: 'Active'}); setEditingId(null); setIsModalOpen(true); }}
                    ...
                    <tbody>
                        {designations.map((desig) => (
                            <tr key={desig.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                                <td className="p-4 text-sm text-slate-500 font-medium">{desig.designation_code}</td>
                                <td className="p-4 font-medium text-slate-800">
                                    {desig.title}
                                    {desig.description && <p className="text-xs text-slate-500 font-normal mt-0.5">{desig.description}</p>}
                                </td>
                                <td className="p-4 text-slate-600">
                                    <span className="bg-slate-100 px-2 py-1 rounded text-sm">{desig.department_name}</span>
                                </td>
                                <td className="p-4">
                                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${desig.status === 'Active' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
                                        {desig.status || 'Active'}
                                    </span>
                                </td>
                                <td className="p-4 text-slate-500 text-sm">{new Date(desig.created_at).toLocaleDateString()}</td>
                                {isAdminOrHR && (
                ...
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col overflow-hidden">
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto">
                            {!editingId && (
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 mb-1">Designation Code (Optional)</label>
                                    <input 
                                        type="text" 
                                        value={formData.designation_code}
                                        onChange={(e) => setFormData({...formData, designation_code: e.target.value})}
                                        placeholder="Auto-generated if left blank"
                                        className="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none text-sm"
                                    />
                                </div>
                            )}
                            <div>
                            ...
                            <div>
                                ...
                                <select 
                                    ...
                                    className="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none bg-white text-sm"
                                >
                            ...
                            <div className="pt-4 flex justify-end space-x-3 shrink-0">
                                <button type="button" onClick={() => setIsModalOpen(false)} className="px-4 py-2 text-slate-600 hover:bg-slate-100 rounded-lg transition-colors text-sm font-medium">Cancel</button>
                                <button type="submit" className="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors text-sm font-medium">Save</button>
                            </div>
                        </form>
                    </div>
                </div>,
